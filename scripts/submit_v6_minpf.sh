#!/usr/bin/env bash
# Submit v6 min-prefill: τ=0.5, C=1/3/5, ρ=0.25/0.5 on AIME, remask only
# Code changed to prefill_mode="min" (legacy behavior)
set -euo pipefail
cd "$(dirname "$0")/.."

VOLC=~/.volc/bin/volc
TMP=/tmp/_submit_v6_minpf.yml
SUBMITTED=0
...
echo "=== v6-minpf: τ=0.5 C=1/3/5 ρ=0.25/0.5 on ${DATASET} (min prefill, remask only) ==="

for CFG in "${CONFIGS[@]}"; do
  read -r TAU C_MAX RHO <<< "$CFG"
  TAG="lowprob_t${TAU}_c${C_MAX}_r${RHO}"
  TASK_NAME="v6mp-${DATASET}-${TAG}"
...
Tags: [LLM, yl, llada, remask, v6, minpf]
