#!/usr/bin/env bash
# Submit v4 eval: lowprob τ=0.5, C=3, ρ=0.25 → results_v4
...
for DATASET in aime2025 gsm_plus humaneval piqa; do
  ...
  for ((S=0; S<NUM_SHARDS; S++)); do
    for MODE in original remask; do
      TASK_NAME="v4-${DATASET}-${MODE}-s${S}of${NUM_SHARDS}"
...
Entrypoint: "cd /vepfs-mlp2/c20250506/251105017/yaolin/LLADA_pretraining/new_llada2.1_infer_remask && export HF_TOKEN=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 && bash scripts/run_v4_single.sh ${DATASET} ${S} ${NUM_SHARDS} ${MODE} 1"
Tags: [LLM, yl, llada, remask, v4]
