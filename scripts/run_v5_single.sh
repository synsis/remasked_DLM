#!/usr/bin/env bash
# Run one shard with lowprob τ=0.3, C_max=3, ρ_max=0.25
# Usage: bash run_v5_single.sh <dataset> <shard_id> <num_shards> <mode> [bsz]
set -euo pipefail
...
DATASET=${1:?Usage: bash run_v5_single.sh <dataset> <shard_id> <num_shards> <mode> [bsz]}
SHARD_ID=${2:?}
NUM_SHARDS=${3:?}
...
OUT_DIR="results_v5/${DATASET}/${TAG}"
mkdir -p "$OUT_DIR"
