...
# Usage: bash run_best_eval_single.sh <dataset> <shard_id> <num_shards> <mode> [bsz]
#
...
DATASET=${1:?Usage: bash run_best_eval_single.sh <dataset> <shard_id> <num_shards> <mode> [bsz]}
SHARD_ID=${2:?}
NUM_SHARDS=${3:?}
...
OUT_DIR="results_v2/best_eval1/${DATASET}/${TAG}"
mkdir -p "$OUT_DIR"
