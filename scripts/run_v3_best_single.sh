#!/usr/bin/env bash
# Run one shard with the "best accuracy" remask setting.
# Usage: bash run_v3_best_single.sh <dataset> <shard_id> <num_shards> <mode> [bsz]
#
# Best accuracy setting: LowProb τ=0.7, C_max=3, ρ_max=1.0
set -euo pipefail
...
DATASET=${1:?Usage: bash run_v3_best_single.sh <dataset> <shard_id> <num_shards> <mode> [bsz]}
SHARD_ID=${2:?}
NUM_SHARDS=${3:?}
...
OUT_DIR="results_v3/${DATASET}/${TAG}"
mkdir -p "$OUT_DIR"
