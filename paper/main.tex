...
\begin{itemize}
    ...
    \item We discover that \emph{last-mile token corruption}---where the model's reasoning is correct but the final answer is corrupted during denoising---is the dominant error mode (73.7\% of errors on CMATH), and show that \TtoM{} specifically repairs this failure mode.
    \item We conduct comprehensive experiments across 12 benchmarks, showing consistent improvements on tasks requiring precise token-level output, with the largest gain on mathematics.
\end{itemize}
...
\begin{table}[t]
\centering
\caption{Main results across 12 benchmarks. All methods use LLaDA2.1-mini with identical inference parameters; \TtoM{} uses the \textsc{LowProb} strategy ($\tau{=}0.3$, $C_{\max}{=}1$, $\rho_{\max}{=}0.25$). TriviaQA and DROP report F1; others report accuracy or pass@1 (\%). Best per-benchmark in \textbf{bold}. ``--'' indicates evaluation in progress.}
\label{tab:main}
\vspace{4pt}
\small
\begin{tabular}{llccc}
...
\multirow{3}{*}{Knowledge}
  & GPQA-Diamond   & 21.7          & 21.7          & $\pm 0.0$ \\
  & TriviaQA (F1)  & 30.5          & \textbf{31.0} & \good{+0.4} \\
  & MMLU-Pro       & --            & --            & -- \\
\midrule
\multirow{4}{*}{Reasoning}
  & HellaSwag      & 78.6          & \textbf{78.7} & \good{+0.1} \\
  & PIQA           & \textbf{82.4} & 81.9          & $-0.4$ \\
  & DROP (F1)      & 38.5          & \textbf{38.9} & \good{+0.4} \\
  & BBH            & --            & --            & -- \\
\midrule
\multirow{3}{*}{Math}
  & CMATH          & 82.3          & \textbf{88.3} & \good{+5.9} \\
  & AIME 2025      & 33.3          & 33.3          & $\pm 0.0$ \\
  & GSM Plus       & \textbf{67.5} & 67.2          & $-0.3$ \\
\midrule
Coding
  & HumanEval+     & \textbf{64.0} & 61.0          & $-3.0$ \\
\midrule
Instruction
  & IFEval (Strict) & 73.0         & \textbf{74.1} & \good{+1.1} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:main} presents the main results. \TtoM{} remasking yields the largest gain on \textbf{CMATH} ($+5.9$), a Chinese elementary math benchmark where final answers are exact numbers and even a single corrupted digit causes failure. Notable improvement is also observed on \textbf{instruction following} (IFEval $+1.1$), where precise format compliance benefits from cleaner context. Modest gains appear on \textbf{extractive QA} (TriviaQA $+0.4$ F1, DROP $+0.4$ F1), where answers are short spans requiring precise token-level reproduction. On \textbf{knowledge} (GPQA $\pm 0.0$), other \textbf{mathematics} benchmarks (AIME~2025 $\pm 0.0$ with only 30 problems, GSM~Plus $-0.3$), and \textbf{commonsense reasoning} (HellaSwag $+0.1$, PIQA $-0.4$), performance is essentially unchanged. On \textbf{coding} (HumanEval+ $-3.0$), performance decreases, likely because remasking disrupts the syntactic token dependencies that code generation relies on---correct code tokens may appear low-confidence in isolation yet be essential for structural validity. We analyze these task-dependent patterns in Section~\ref{sec:analysis}.
...
We conduct a systematic error analysis on CMATH (1,098 elementary math problems), where \TtoM{} achieves the largest gain ($+5.9\%$). Among the 194 errors made by \TtoT{}, \textbf{143 (73.7\%)} are \emph{last-mile token corruption}: the model's chain-of-thought reasoning arrives at the correct answer, yet the final extracted answer is corrupted during denoising. Only 51 errors (26.3\%) are genuine reasoning failures. Corruption takes several characteristic forms: \textbf{extra digits appended} ($18 \to 188$, 44 cases), \textbf{digits dropped} ($32 \to 2$, 26 cases), \textbf{answer marker repetition} (25 cases), and \textbf{single-digit substitution} (14 cases).

\TtoM{} remasking repairs \textbf{97 of 143} corruption cases (67.8\%), reducing total corruption errors from 143 to 76 ($\downarrow$46.9\%), while genuine reasoning errors remain virtually unchanged (51$\to$53). Table~\ref{tab:corruption} provides the full breakdown; representative examples are shown in Figure~\ref{fig:case_study} (Appendix~\ref{app:cases}).
...
\midrule
\TtoT{} editing   & 194  & 143 ~(73.7\%)  & 51 ~(26.3\%)  & 82.3\% \\
\TtoM{} remasking  & 129  & ~76 ~(58.9\%)  & 53 ~(41.1\%)  & 88.3\% \\
\midrule
$\Delta$           & $-65$ & $-67$ ~($\downarrow$46.9\%) & $+2$ & $+5.9\%$ \\
\bottomrule
\end{tabular}
\end{table}

Among the 108 samples that \TtoM{} flipped from incorrect to correct, 97 (89.8\%) are corruption fixes and only 11 (10.2\%) reflect improved reasoning. Conversely, \TtoM{} introduces 43 new errors, of which 30 are new corruption cases---indicating that remasking occasionally destabilizes the denoising process at the answer boundary, though far less often than it helps.

The step-by-step token denoising trajectories for representative examples (DROP 160 and DROP 75) are provided in Figure~\ref{fig:trajectory} (Appendix~\ref{app:trajectory}), showing per-token probabilities at each denoising iteration for both \TtoT{} and \TtoM{}.
...
Our theoretical analysis provides a unified understanding of why mask tokens are strictly preferable to erroneous tokens as context, drawing parallels between the discrete and continuous diffusion settings. Three complementary error detection strategies---\textsc{LowProb}, \textsc{T2T-Remask}, and \textsc{LogitDiff}---offer flexibility in detecting different types of token-level errors. Experiments across 12 benchmarks show consistent improvements on tasks requiring precise token-level output, with the largest gain on mathematics; our error analysis reveals that last-mile token corruption accounts for 73.7\% of errors on CMATH, with \TtoM{} repairing 67.8\% of such corruptions.
