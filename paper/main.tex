...
\begin{figure}[t]
    \centering
    \includegraphics[width=\textwidth, trim=135pt 50pt 130pt 35pt, clip]{figures/fig1.pdf}
    \caption{\textbf{T2T editing vs.\ T2M remasking.}
    (a)~T2T detects the erroneous token ``purple'' but cannot correct it:
    the conditional distribution is multimodal (``happy,'' ``glad,'' ``pleased'' each ${\sim}0.25$)
    so no single candidate exceeds the confidence threshold---a failure mode we call
    \emph{correction inertia}.  The error persists and pollutes context for subsequent predictions.
    (b)~T2M detects the same error via low posterior probability and resets it to \mask{}
    (\emph{context purification}).  A subsequent M2T step re-predicts the position under
    cleaner context, producing the correct token (\emph{delayed commitment}).}
    \label{fig:overview}
  \end{figure}
...
\begin{itemize}
    ...
    \item We conduct comprehensive experiments across \textsc{XX} benchmarks demonstrating consistent improvements.
\end{itemize}
...
\noindent This situation arises naturally when the conditional distribution $p_\theta(\cdot \mid \mathbf{z}_{-i})$ is multimodal. For instance, consider completing ``He felt very \underline{\hspace{1cm}}'': the tokens ``happy,'' ``excited,'' and ``pleased'' may each receive probability $\sim$0.25, while the incorrect committed token ``purple'' has probability $\sim$0.01. \TtoT{} with $\tau_{\text{t2t}} = 0.5$ takes no action since no single candidate exceeds the threshold, despite the model clearly recognizing the error.
...
Under this hierarchy, \TtoM{} replaces type~(3) signals with type~(2), eliminating sources of adversarial bias at zero cost (null signals are strictly non-harmful). \TtoT{} replaces type~(3) with a new token that may itself be type~(3), providing no guarantee of net improvement.
...
\paragraph{Benchmarks.} We evaluate on a comprehensive suite of benchmarks spanning four categories: \textbf{Knowledge} (MMLU-Pro~\citep{wang2024mmlu_pro}, GPQA-Diamond~\citep{rein2024gpqa}, TriviaQA~\citep{joshi2017triviaqa}), \textbf{Reasoning} (BBH~\citep{suzgun2022bbh}, HellaSwag~\citep{zellers2019hellaswag}, PIQA~\citep{bisk2020piqa}, DROP~\citep{dua2019drop}), \textbf{Mathematics} (GSM-Plus~\citep{li2024gsm_plus}, CMATH~\citep{wei2024cmath}, AIME~2025~\citep{aime2025}, OlympiadBench~\citep{he2024olympiadbench}), and \textbf{Coding} (HumanEval+~\citep{chen2021humaneval}, MBPP+~\citep{austin2021mbpp}).

\subsection{Main Results}

\begin{table}[t]
\centering
\caption{Main results on knowledge and reasoning benchmarks. All methods use LLaDA2.1-mini with identical inference parameters. Best result in \textbf{bold}.}
\label{tab:main_kr}
\vspace{4pt}
\begin{tabular}{lccccccc}
\toprule
\textbf{Method} & \textbf{MMLU-Pro} & \textbf{GPQA} & \textbf{TriviaQA} & \textbf{BBH} & \textbf{HellaSwag} & \textbf{PIQA} & \textbf{DROP} \\
\midrule
Original (T2T) & -- & -- & -- & -- & -- & -- & -- \\
\midrule
\textsc{LowProb}     & -- & -- & -- & -- & -- & -- & -- \\
...
\begin{table}[t]
\centering
\caption{Main results on mathematics benchmarks.}
\label{tab:main_math}
\vspace{4pt}
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{GSM-Plus} & \textbf{CMATH} & \textbf{AIME 2025} & \textbf{OlympiadBench} \\
\midrule
Original (T2T) & -- & -- & -- & -- \\
\midrule
\textsc{LowProb}     & -- & -- & -- & -- \\
...
\begin{table}[t]
\centering
\caption{Main results on coding benchmarks.}
\label{tab:main_code}
\vspace{4pt}
\begin{tabular}{lcc}
\toprule
...
\paragraph{Remask statistics.}
We report the average number of remasked tokens per block and per generation across benchmarks (Table~\ref{tab:remask_stats}).

\begin{table}[t]
\centering
\caption{Average remask statistics across benchmarks.}
\label{tab:remask_stats}
\vspace{4pt}
\begin{tabular}{lccc}
\toprule
\textbf{Strategy} & \textbf{Remasks / block} & \textbf{Remasks / generation} & \textbf{Time overhead (\%)} \\
\midrule
\textsc{LowProb}     & -- & -- & -- \\
\textsc{T2T-Remask}   & -- & -- & -- \\
\textsc{LogitDiff}    & -- & -- & -- \\
...
Since \TtoM{} operates within the existing generation loop and reuses the same forward pass as \TtoT{}, it introduces minimal computational overhead. The additional cost comes from potentially more inner-loop iterations (remasked positions require re-prediction). We report wall-clock time comparisons in Table~\ref{tab:remask_stats}.

\paragraph{Qualitative analysis.}
We provide case studies in Appendix~\ref{app:cases} showing examples where \TtoT{} converges to an incorrect fixed point while \TtoM{} successfully corrects the error through context purification.
...
\section{Case Study: Last-Mile Token Corruption on CMATH}
\label{app:cases}

We conduct a systematic error analysis on CMATH (1,097 elementary math problems) to understand \emph{why} \TtoM{} remasking improves accuracy. The majority of \TtoT{} errors are not reasoning failures but \emph{last-mile token corruption}---the model reasons correctly yet the final numeric answer is corrupted during denoising. \TtoM{} remasking substantially reduces such corruption while leaving genuine reasoning errors unchanged.

\paragraph{The last-mile corruption problem.}
Among the 188 errors made by \TtoT{} editing, we find that in \textbf{139 cases (73.9\%)} the model's step-by-step reasoning arrives at the correct numeric answer, yet the final output (the answer extracted for evaluation) is corrupted. Only 49 errors (26.1\%) are genuine reasoning mistakes.

This corruption takes several characteristic forms: \textbf{extra digits appended} ($17 \to 177$), \textbf{leading or trailing digits dropped} ($31 \to 1$, $40 \to 4$), \textbf{answer marker repetition} (``the answer is the answer is\ldots''), and \textbf{single-digit substitution}.

\paragraph{How remasking reduces corruption.}
Table~\ref{tab:corruption} shows that \TtoM{} remasking reduces corruption cases by 47.5\% (139$\to$73) while genuine reasoning errors remain essentially unchanged (49$\to$51). This confirms that the accuracy improvement (+5.8\%) comes almost entirely from eliminating token-level denoising artifacts.

\begin{table}[h]
\centering
\caption{Error breakdown on CMATH: \TtoT{} editing vs.\ \TtoM{} remasking (\textsc{LowProb}). ``Corruption'' denotes cases where the correct answer appears in the reasoning chain but the final extracted answer is wrong.}
...
The most dramatic improvements are in the two dominant corruption types: extra-digit appending drops from 43 to 5 cases ($\downarrow$88\%), and answer-marker repetition is completely eliminated (11$\to$0).

\paragraph{Representative examples.}
Figure~\ref{fig:case_study} shows three representative cases. In each case the model performs correct arithmetic in its chain-of-thought, but \TtoT{} editing fails to prevent the final answer from being corrupted. \TtoM{} remasking, by resetting low-confidence tokens to \mask{} and re-predicting them, produces the correct final answer.
