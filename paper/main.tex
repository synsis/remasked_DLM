...
\begin{table}[t]
\centering
\caption{Main results across 12 benchmarks. All methods use LLaDA2.1-mini with identical inference parameters. T2M~Eff.\ uses \textsc{LowProb} ($\tau{=}0.3$, $C_{\max}{=}1$, $\rho_{\max}{=}0.25$); T2M~Best uses \textsc{LowProb} ($\tau{=}0.7$, $C_{\max}{=}3$, $\rho_{\max}{=}1.0$). TriviaQA and DROP report F1; others report accuracy or pass@1 (\%). Best per-benchmark in \textbf{bold}. ``--'' indicates evaluation in progress or not applicable.}
\label{tab:main}
\vspace{4pt}
\small
\begin{tabular}{llcccc}
\toprule
\textbf{Category} & \textbf{Benchmark} & \textbf{Original (T2T)} & \textbf{T2M (Eff.)} & \textbf{T2M (Best)} \\
\midrule
\multirow{3}{*}{Knowledge}
  & GPQA-Diamond   & 21.72          & 21.72          & -- \\
  & TriviaQA (F1)  & 30.54          & \textbf{30.96} & -- \\
  & MMLU-Pro       & 58.78          & \textbf{58.84} & -- \\
\midrule
\multirow{4}{*}{Reasoning}
  & HellaSwag      & 78.57          & \textbf{78.67} & -- \\
  & PIQA           & \textbf{82.37} & 81.94          & 82.3 \\
  & DROP (F1)      & 38.53          & \textbf{38.90} & -- \\
  & BBH            & \textbf{75.50} & 75.38          & -- \\
\midrule
\multirow{3}{*}{Math}
  & CMATH          & 82.33          & \textbf{88.25} & -- \\
  & AIME 2025      & 33.33          & 33.33          & -- \\
  & GSM Plus       & \textbf{67.54} & 67.21          & 67.9 \\
\midrule
Coding
  & HumanEval+     & \textbf{64.0} & 61.0          & 61.6 \\
\midrule
Instruction
  & IFEval (Strict) & 73.0         & \textbf{74.1} & -- \\
\bottomrule
\end{tabular}
\end{table}
