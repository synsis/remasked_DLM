...
\begin{itemize}
    ...
    \item We discover that \emph{last-mile token corruption}---where the model's reasoning is correct but the final answer is corrupted during denoising---is the dominant error mode (73.9\% of errors on CMATH), and show that \TtoM{} specifically repairs this failure mode.
    \item We conduct comprehensive experiments across 9 benchmarks demonstrating broad improvements, particularly on tasks requiring precise token-level output.
\end{itemize}
...
\paragraph{Evaluation note.} We use the official LLaDA2.1 model weights and official benchmark datasets. As LLaDA2.1 does not release its evaluation code, our reproduced baseline absolute scores may differ from the original paper due to prompt template or answer extraction differences; all \TtoT{} vs.\ \TtoM{} comparisons use \emph{identical} evaluation pipelines.

\paragraph{Benchmarks.} We evaluate on 9 benchmarks spanning five categories: \textbf{Knowledge} (GPQA-Diamond~\citep{rein2024gpqa}, TriviaQA~\citep{joshi2017triviaqa}), \textbf{Reasoning} (HellaSwag~\citep{zellers2019hellaswag}, PIQA~\citep{bisk2020piqa}, DROP~\citep{dua2019drop}), \textbf{Mathematics} (CMATH~\citep{wei2023cmath}, AIME~2025~\citep{aime2025}), \textbf{Coding} (HumanEval+~\citep{chen2021humaneval,liu2023evalplus}), and \textbf{Instruction Following} (IFEval~\citep{zhou2023ifeval}).

\subsection{Main Results}

\begin{table}[t]
\centering
\caption{Main results across 9 benchmarks. All methods use LLaDA2.1-mini with identical inference parameters; \TtoM{} uses the \textsc{LowProb} strategy ($\tau{=}0.3$, $C_{\max}{=}1$, $\rho_{\max}{=}0.25$). TriviaQA and DROP report F1; others report accuracy or pass@1 (\%). Best per-benchmark in \textbf{bold}.}
\label{tab:main}
\vspace{4pt}
\small
\begin{tabular}{llccc}
\toprule
\textbf{Category} & \textbf{Benchmark} & \textbf{Original (T2T)} & \textbf{T2M (LowProb)} & $\boldsymbol{\Delta}$ \\
\midrule
...
\midrule
Instruction
  & IFEval (Strict) & 73.0         & \textbf{73.4} & \good{+0.4} \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:main} presents the main results. \TtoM{} remasking yields the largest gains on \textbf{mathematics} tasks (CMATH $+5.8$, AIME~2025 $+6.7$\footnote{AIME~2025 contains only 30 problems; the $+6.7\%$ gain corresponds to 2 additional correct answers and should be interpreted with caution given the small sample size.}), where final answers are exact numbers and even a single corrupted digit causes failure. Improvements are also observed on \textbf{knowledge and extractive QA} (GPQA $+1.0$, TriviaQA $+1.2$ F1, DROP $+0.6$ F1), where answers are short spans requiring precise token-level reproduction, and on \textbf{instruction following} (IFEval $+0.4$). On \textbf{commonsense reasoning} tasks (HellaSwag, PIQA), performance is essentially unchanged ($-0.1$/$-0.1$): outputs are short ($\sim$100--140 tokens) and rarely suffer from corruption. On \textbf{coding} (HumanEval+), no difference is observed ($\pm 0.0$). We analyze these task-dependent patterns in Section~\ref{sec:analysis}.
...
Figure~\ref{fig:ablation} plots each configuration as a point. The $x$-axis measures average token modifications per sample---T2T edits for the baseline (${\star}$, 12.4 edits/sample), remask count for \TtoM{} strategies. The $y$-axis measures accuracy.
...
\noindent Overall, the most cost-efficient configuration (\textsc{LowProb} $\tau{=}0.3$, $C{=}1$, $\rho{=}0.25$) adds only ${\sim}69$ remasks per sample---modest relative to the average output length of ${\sim}333$ tokens---while achieving a 12-point accuracy gain over \TtoT{}.

\subsection{Analysis}
\label{sec:analysis}

\paragraph{Per-sample flip analysis.}
To understand \emph{how} \TtoM{} improves accuracy, we classify every sample into four categories: both methods correct, both wrong, only \TtoT{} correct (``T2T only''), or only \TtoM{} correct (``T2M only''). Table~\ref{tab:flip} reports this breakdown. On every benchmark, the vast majority of samples (90--99\%) are unaffected---improvements come from a small fraction where remasking flips the outcome.

\begin{table}[t]
\centering
\caption{Per-sample flip analysis. ``T2T only'' = \TtoT{} correct but \TtoM{} wrong; ``T2M only'' = the reverse. Net gain = T2M only $-$ T2T only.}
\label{tab:flip}
\vspace{4pt}
\small
\begin{tabular}{lrrrrrr}
\toprule
\textbf{Benchmark} & \textbf{N} & \textbf{Both correct} & \textbf{Both wrong} & \textbf{T2T only} & \textbf{T2M only} & \textbf{Net} \\
\midrule
...
\begin{itemize}[nosep,leftmargin=*]
    ...
    \item \textbf{Extractive QA} (DROP $+0.6$ F1, TriviaQA $+1.2$ F1): answers are short spans where token-level fidelity matters (e.g., ``Derrick'' $\to$ ``Derrick Mason'', ``Basket'' $\to$ ``Basketball'').
\end{itemize}
\TtoM{} shows a modest gain on \textbf{instruction following} (IFEval $+0.4$), and no meaningful benefit on \textbf{commonsense reasoning} tasks (HellaSwag $-0.1$, PIQA $-0.1$) or \textbf{coding} (HumanEval+ $\pm 0.0$). These latter tasks either produce short outputs ($\sim$105--141 tokens for commonsense) or require structural correctness rather than exact token fidelity (coding), leaving little room for last-mile corruption. For HellaSwag/PIQA, only 4--5\% of samples change prediction at all, and the flips are nearly symmetric (HellaSwag: 194 broke vs.\ 178 fixed), indicating that remasking acts as random perturbation rather than targeted correction on these tasks.
...
Our theoretical analysis provides a unified understanding of why mask tokens are strictly preferable to erroneous tokens as context, drawing parallels between the discrete and continuous diffusion settings. Three complementary error detection strategies---\textsc{LowProb}, \textsc{T2T-Remask}, and \textsc{LogitDiff}---offer flexibility in detecting different types of token-level errors. Experiments across 9 benchmarks demonstrate broad improvements, with the largest gains on tasks requiring precise token-level output; our error analysis reveals that last-mile token corruption accounts for 73.9\% of errors on CMATH, with \TtoM{} repairing 67.6\% of such corruptions.
