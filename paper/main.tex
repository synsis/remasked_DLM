...
\begin{figure}[t]
    ...
    \caption{\textbf{Token modification cost vs.\ accuracy.} Each point is one (strategy, $\tau$, $C_{\max}$, $\rho_{\max}$) configuration on CMATH (100 samples). The $x$-axis measures average token modifications per sample: T2T edits for the baseline, remasks for \TtoM{} strategies. Color shade encodes $\tau$ (lighter = smaller). The vertical dotted line marks the average output length (${\sim}334$ tokens). All \TtoM{} strategies surpass the T2T baseline (81\%, ${\star}$). \textbf{Best accuracy}: \textsc{LowProb} $\tau{=}0.7$, $C{=}3$, $\rho{=}1.0$ (95\%, 523.8 remasks). \textbf{Most efficient}: \textsc{LowProb} $\tau{=}0.3$, $C{=}1$, $\rho{=}0.25$ (93\%, only 69.2 remasks). \textbf{Cheapest}: \textsc{T2T-Remask} $\tau{=}0.9$, $C{=}1$, $\rho{=}0.5$ (87\%, 1.7 remasks).}
    \label{fig:ablation}
\end{figure}
...
\begin{itemize}[nosep,leftmargin=*]
    ...
    \item \textbf{\textsc{LowProb} dominates.} Across all cost levels, \textsc{LowProb} achieves the highest accuracy. At low cost, $\tau{=}0.3$, $C{=}1$, $\rho{=}0.25$ reaches 93\% with only 69.2 remasks; at higher cost, $\tau{=}0.7$, $C{=}3$, $\rho{=}1.0$ reaches 95\% with 523.8 remasks.
