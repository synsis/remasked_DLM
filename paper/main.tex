...
\paragraph{Per-position remask budget.} Each position $i$ maintains a counter $c_i$, incremented each time it is remasked. Remasking is blocked when $c_i \geq C_{\max}$ (default $C_{\max} = 5$), preventing infinite cycles.
...
\paragraph{Remask hyperparameters.} For \TtoM{} strategies, we use the following default thresholds (tuned on a 50-sample subset of GSM-Plus): \textsc{LowProb} $\tau_{\textsc{lp}} = 0.5$, \textsc{T2T-Remask} $\tau_{\textsc{tr}} = 0.9$, \textsc{LogitDiff} $\tau_{\textsc{ld}} = 0.3$. Safety parameters: $C_{\max} = 5$ (per-position budget), $\rho_{\max} = 0.25$ (ratio cap).
...
\midrule
$C_{\max}=5$, $\rho_{\max}=0.25$ (default) & -- & -- \\
$C_{\max}=3$, $\rho_{\max}=0.25$            & -- & -- \\
$C_{\max}=\infty$, $\rho_{\max}=0.25$       & -- & -- \\
$C_{\max}=5$, $\rho_{\max}=0.50$            & -- & -- \\
$C_{\max}=5$, $\rho_{\max}=1.0$             & -- & -- \\
\bottomrule
\end{tabular}
\end{table}
